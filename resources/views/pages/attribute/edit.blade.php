...
<div class="form-box">
    <form action="{{ route('attribute.update', $attribute->id) }}" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="name" value="{{ old('name', $attribute->name) }}" placeholder="Enter name">
        </div>
        <div class="form-group">
        ...
        <div id="attribute-list" class="mt-2">
            @foreach($attribute->options as $option)
                <div class="attribute-item">
                    <span>{{ $option }}</span>
                    <input type="hidden" name="options[]" value="{{ $option }}">
                    <span class="cancel-icon" onclick="removeAttribute(this)">&times;</span>
                </div>
            @endforeach
        </div>
        ...
        <script>
            ...
            function removeAttribute(element) {
                const attributeList = document.getElementById('attribute-list');
                const attributeItem = element.parentElement;
                attributeList.removeChild(attributeItem);
            }
        </script>
        ...
        <div class="form-group">
            ...
            <div class="dropdown">
                <select name="active" class="form-control" id="status">
                    <option value="1" {{ old('active', $attribute->active) == 1 ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ old('active', $attribute->active) == 0 ? 'selected' : '' }}>In-Active</option>
                </select>
