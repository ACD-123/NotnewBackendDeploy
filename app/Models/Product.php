...
class Product extends Base implements IMediaInteraction
{
    ...
    'model', 'brand', 'stockcapacity', 'selling_now','auction_listing', 'auction_End_listing',
    ...
    'available_colors', 'shop_id','return_ship_duration_limt','return_ship_location','return_ship_paid_by','hot','underage','used_condition','is_deleted'];
    
    protected $appends = ['cover_image', 'is_owner','is_favourite','favourite_count','wishlist','wishlist_count','total_bids','max_user','max_bid','average_rating_count','dis_percentage'];
...
//        }]);
    ...
    public function getDisPercentageAttribute()
    {
        if ($this->sale_price != NULL && !empty($this->sale_price) && $this->sale_price>0) {

            $difference =(($this->price - $this->sale_price) / ($this->price)) * 100;
    ...
    {
        if (Auth::guard('api')->check()) {
           return Favourite::isFavourite($this->guid,'1',Auth::guard('api')->id());
        }
        return false;
    }
    ...
    {
        if ($this->auctioned==1) {
           return Bidding::getTotalBids($this->id);
        }
        return 0;
    }
    ...
    {
        if ($this->auctioned==1) {
           return Bidding::getMaxBid($this->id);
        }
        return 0;
    }

    public function getMaxUserAttribute()
    {
        if ($this->auctioned==1) {
           return Bidding::getMaxBidUser($this->id);
        }
        return null;
    }
    ...
    {
        $totalRatings = FeedBack::where('product_id', $this->id)->sum('ratings');
        $ratingsCount = FeedBack::where('product_id', $this->id)->count();
    ...
    {
        if (Auth::guard('api')->check()) {
           return Wishlist::isWishlist($this->guid,Auth::guard('api')->id());
        }
        return false;
    }
    ...
    {
        return Wishlist::wishlistCount($this->guid);
    }

    public function getFavouriteCountAttribute()
    {
        return Favourite::favouriteCount($this->guid,'1');
    }
